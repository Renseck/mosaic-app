...
pub fn dashboard_list_page() -> Html {
    ...
    html! {
        <div class="max-w-5xl mx-auto">
            ...
            /* ====== Inline create form ====== */
            if *show_form {
                <div class="mb-6 bg-white border border-stone-200 rounded-lg p-4 shadow-sm">
                    <form onsubmit={on_create} class="flex items-center gap-3">
                        ...
                        <button
                            ...
                            class="px-4 py-2 text-sm font-semibold text-slate-900 bg-amber-500
                                   rounded-md hover:bg-amber-400 disabled:opacity-50 transition-colors"
                        >
            ...
            if state.loading && state.data.is_none() {
                ...
                if dashboards.is_empty() {
                    <div class="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p class="text-stone-500 text-sm">{"No dashboards yet."}</p>
                        <p class="text-stone-400 text-xs mt-1">{"Create one to get started."}</p>
                    </div>
                } else {
...
fn dashboard_card(props: &DashboardCardProps) -> Html {
    ...
    html! {
        <Link<Route>
            ...
            classes="block group bg-white border border-stone-200 rounded-lg p-5 shadow-sm
                     hover:border-amber-400 hover:shadow-md transition-all"
        >
            <div class="flex items-start gap-3">
                <div class="w-9 h-9 rounded-lg bg-amber-100 flex items-center justify-center
                            text-amber-700 text-base shrink-0">
                    { d.icon.as_deref().unwrap_or("▦") }
                </div>
                <div class="min-w-0">
                    <p class="text-sm font-semibold text-stone-900 group-hover:text-amber-700
            ...
            if d.is_shared {
                <span class="mt-3 inline-block text-xs text-amber-600 font-medium">{"Shared"}</span>
            }
        </Link<Route>>
    }
}
