...
- GRAFANA_SERVICE_ACCOUNT_TOKEN=glsa_xxxxxxxxxxxx --> (POST `/api/serviceaccounts`)
...
- GRAFANA_DATASOURCE_UID=<your-datasource-uid> --> (POST `/api/datasources`)
```json
...
I hope some of these may be doable through clever Docker configuration, where we spawn the instances with certain things already provisioned. If not, we'll have to come up with some other way.
...
Ideally, simply running `make docker-up` leaves us with a fully configured, ready-to-rumble platform. Perhaps a general Bootstrapper can take care of all or most of these things? Do you have ideas on how to solve these problems, to make bootstrapping as clean and reproducible as possible?
